...
export FLAVOR_CHROOT_SCRIPTS="sddm bluetooth sshd systemd-timesyncd NetworkManager ima-offload.target var-lib-pacman.mount nix.mount opt.mount root.mount srv.mount usr-lib-debug.mount usr-local.mount var-cache-pacman.mount var-lib-docker.mount var-lib-flatpak.mount var-lib-systemd-coredump.mount var-log.mount var-tmp.mount powerbutton-chmod"
#export FLAVOR_PLYMOUTH_THEME="steamos"
export FLAVOR_FINAL_DISTRIB_IMAGE=$FLAVOR_BUILDVER
export KERNELCHOICE="linux-lts"
export BASE_BOOTSTRAP_PKGS="base base-devel flatpak arch-install-scripts linux-firmware amd-ucode intel-ucode sddm dkms jq btrfs-progs grub efibootmgr openssh"
export UI_BOOTSTRAP="plasma plasma-nm dolphin konsole sddm mesa vulkan-radeon vulkan-intel vulkan-nouveau"
export BUILDER_RELEASE="IMAGE_ID=\"${FLAVOR_BUILDVER}\"\nOS_TAG=${RELEASETAG}\nRELEASETYPE=$(echo ${OS_CODENAME} | tr '[:upper:]' '[:lower:]')\nISINTERNAL=no"
